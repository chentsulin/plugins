...
use swc_common::{chain, comments::Comments, SourceFile};
...
///
/// Only [analyzer], [display_name_and_id], [transpile_css_prop] and [pure] are implemented.
pub fn styled_components<C: Comments>(comments: Option<C>, file: Arc<SourceFile>, config: Config) -> impl Fold + VisitMut {
    ...
    chain!(
        ...
        transpile_css_prop(),
        pure(comments, config.clone(), state.clone())
    )
}
